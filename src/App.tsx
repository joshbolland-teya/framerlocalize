...
export function App() {
    ...
    const localizationModeHint = "Open this plugin from the Localizations panel to sync translations."
    ...
    const [selectedLocaleCodes, setSelectedLocaleCodes] = useState<Set<string>>(new Set());
    ...
    useEffect(() => {
      ...
      (async () => {
        ...
        // force rerender without referencing selectedGroupIds
        setSelectedGroupIds(s => new Set(s));
      })();
    }, []);
    ...
    const handleExport = async () => {
        ...
        try {
            ...
            const localeCols = locales.filter(l =>
            ...
            const filteredGroups = groups.filter(g =>
                selectedGroupIds.size === 0 || selectedGroupIds.has(g.id)
            );
            filteredGroups.forEach(group => {
                ...
                group.sources.forEach(source => {
                    ...
                    localeCols.forEach(locale => {
                        const localized =
                            source.valueByLocale?.[locale.id]?.value ?? ""
    ...
    const handleExportXLIFF = async () => {
      ...
      try {
        ...
        const selectedLocales = locales.filter(l =>
          selectedLocaleCodes.size === 0 || selectedLocaleCodes.has(l.code)
        );
        ...
        // For each locale, generate ONE XLIFF file
        for (const locale of selectedLocales) {
          ...
          filteredGroups.forEach(group => {
            group.sources.forEach(source => {
              const baseValue = source.value ?? "";
              const safeBase = baseValue.replace(/&/g, "&amp;").replace(/</g, "&lt;");
              ...
              // No <note> tag included (per requirements)
              xml += `      </trans-unit>\n`;
            });
          });
        ...
        notify("XLIFF 1.2 files generated.", "success");
      } catch (err) {
            ...
            for (const row of rows) {
                ...
                if (existing) {
                    if ((existing.value ?? "").length > 32767) {
                        console.warn(`Skipping sourceId ${sourceId}: existing baseValue too long`);
                        continue;
                    }
                    ...
                    if (hasOversized) {
                        console.warn(`Skipping sourceId ${sourceId}: existing localized value too long`);
                        continue;
                    }
                }
                ...
                Object.entries(row as ExcelRow).forEach(([key, val]) => {
                    if (key === "sourceId" || key === "groupId" || key === "groupName" || key === "baseValue") {
                        return
                    }
    ...
    return (
        ...
        <main className="app-shell">
            <header>
                <h1>Framer Localization Sync</h1>
                <p style={{ color: "var(--framer-color-text-secondary)" }}>
                  Export and re-import translations using Excel or XLF/XLIFF.
                </p>
            </header>
            ...
            <div className="filters">
              {isLoadingFilters && <p style={{ marginBottom: "10px" }}>Loading…</p>}
              {!isLoadingFilters && (
                <>
                  <div style={{ display: "flex", gap: "8px", marginBottom: "10px" }}>
                    <button
                      ...
                      style={{ width: "150px" }}
                      onClick={() => {
                        const all = new Set(groupsRef.current.map(g => g.id));
                  ...
                  {[...groupsRef.current]
                    ...
                    .map((group: LocalizationGroup) => (
                      <label key={group.id} style={{ display: "flex", marginBottom: "8px", alignItems: "center" }}>
                        <input
                          type="checkbox"
                        ...
                        />
                        <span style={{ color: "var(--framer-color-text-primary)" }}>{group.name}</span>
                      </label>
                  ))}

                  <h3 style={{ marginTop: "20px" }}>Filter Languages</h3>
                  {localesRef.current?.map((locale: Locale) => (
                    <label key={locale.code} style={{ display: "flex", marginBottom: "8px", alignItems: "center" }}>
                      <input
                        type="checkbox"
                        style={{ marginRight: "6px" }}
                        ...
                        onChange={() => {
                          ...
                          if (next.has(locale.code)) next.delete(locale.code);
                          else next.add(locale.code);
                          setSelectedLocaleCodes(next);
                        }}
                      />
                      <span style={{ color: "var(--framer-color-text-primary)" }}>{locale.code}</span>
                    </label>
                  ))}
                </>
              )}
...
>
  ...
  {/* Hidden file pickers — keep them functional but not visible */}
  <input
    type="file"
    accept=".xlf,.xliff"
    hidden
  ...
  />
            ...
            <p className="status" style={{ color: "var(--framer-color-text-secondary)" }}>
              {statusMessage}
            </p>
        </main>
        </div>
